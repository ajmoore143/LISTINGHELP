...
1. PRODUCT INPUT
2. RESEARCH SUMMARY
3. KEYWORD PLACEMENT PLAN, grouped into TITLE keywords, BULLET keywords,
   BACKEND keywords, and the full ALL-keywords set.

GENERAL SEO RULES
- Titles, bullet points, and description must all use keywords.
- Use the TITLE keywords in the titles, placing the strongest ones early.
- Integrate the BULLET keywords naturally into the bullet points.
- The description must achieve the broadest keyword coverage: it must
  incorporate the ENTIRE ALL-keywords set, not just the backend or leftover
  terms.
...
- Use the TITLE keywords from the placement plan in the titles.
...
- The description is the broad-coverage zone: it MUST incorporate every keyword
  in the ALL-keywords set, including title, bullet, and backend keywords.
- Weave the keywords in as naturally as possible; the paragraph should still
  read coherently rather than as a keyword list.
- Prioritize complete keyword coverage over hitting a fixed length. Use an
  internal target of about 1000 characters, but expand as needed so that no
  keyword from the ALL set is left out.
